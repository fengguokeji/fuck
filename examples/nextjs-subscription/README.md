...
- 下单后自动识别桌面/移动端：桌面展示二维码供扫码，移动端直接尝试唤起支付宝客户端，并在支付完成后自动跳转订单详情。
...
2. 若配置了真实密钥，将看到支付宝官方二维码；桌面端可直接扫码，移动端会自动唤起支付宝客户端，支付完成后页面会自动跳转到订单详情。
...
- `GET /api/orders/[orderId]?email=<下单邮箱>`：校验邮箱后返回订单当前状态、二维码与最新更新时间，供前端轮询。下单页会调用该接口以判断是否需要刷新二维码或跳转详情页。
